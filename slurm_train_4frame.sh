...
#SBATCH --nodes=8                    # 4 nodes × 2 GPUs/node = 8 GPUs total
#SBATCH --ntasks=8                   # one task per GPU
#SBATCH --ntasks-per-node=1          # 2 tasks (GPUs) on each node
#SBATCH --cpus-per-task=8            # e.g. 8 CPUs per task
#SBATCH --gres=gpu:1                 # request 2 GPUs per node
#SBATCH --mem=24G                    # Memory per node (replacing mem-per-gpu)
#SBATCH --time=30:00:00              # hh:mm:ss
#SBATCH --job-name=petr3d_4frame     # job name
